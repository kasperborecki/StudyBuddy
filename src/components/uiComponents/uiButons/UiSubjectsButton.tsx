...
const UiWhiteButtonLong: React.FC<CustomButtonProps> = ({
  ...
  return (
    <div
      className={`relative bg-opacity-20 w-40 m-3 h-44 rounded-3xl font-roboto text-[#212427be]
      ${colour?.length != 0 && 'shadow-md shadow-bottom'}
      ...
      {colour?.length != 0 && (
        <>
          ...
          <img
            src={CDNURL + icon}
            alt='fizyka'
            className={`w-28 h-28 opacity-70 absolute right-0 top-8 ${
              text === 'Matematyka' ? 'mt-1' : ''
            }`}
          />
        </>
      )}
